...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    denom = np.apply_along_axis(lambda x: np.sqrt(x.dot(x.T)), 1, x)
    x /= denom[:, None]
    ### END YOUR CODE
    return x
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    ### YOUR CODE HERE
    grad = np.zeros(outputVectors.shape)  # WxD
    gradPred = np.zeros(predicted.shape)                   # Dx1
    cost = 0
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    cost = 0.0
    gradIn = np.zeros(inputVectors.shape)   # WxD
    gradOut = np.zeros(outputVectors.shape)  # WxD
...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    cost, gradIn_j, gradOut = word2vecCostAndGradient(predicted, target, outputVectors, dataset)
